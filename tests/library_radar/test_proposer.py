...
def _isolated_bridge(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Route all proposal_bridge writes into a tmp directory."""
    monkeypatch.setenv("PROPOSAL_BRIDGE_DIR", str(tmp_path / "proposal_bridge"))
    yield


def _bridge_proposals():
    from app.proposal_bridge import list_proposals
    return list_proposals(source="library_radar")
...
def test_writes_proposal_with_expected_sections(tmp_path: Path) -> None:
    ...
    assert proposals[0].target_path.startswith("docs/proposed_libraries/")
...
def test_dedups_by_signature(tmp_path: Path) -> None:
    ...
    # Single staged proposal — bridge dedup at signature level.
