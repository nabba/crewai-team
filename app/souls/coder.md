...
- **tool_search**: Search the registry by capability tag and/or natural-language intent. Returns ranked candidates with a one-line reason per match. USE THIS before assuming a tool doesn't exist — the registry is the source of truth, and includes Forge-generated tools an operator may have added. Pattern: `tool_search(intent="render forest deforestation PDF", capabilities=["renders-pdf"])`.
- **load_tool / list_available_tools** (LoadableAgent only — present when `LOADABLE_CODER=1`): when the agent ships with a discoverable catalog, `list_available_tools` shows what's loadable on demand and `load_tool(name=...)` pulls one into the active toolset for the next step. Use `tool_search` first to find what you need; `load_tool` to commit.
