...
def _load_relevant_team_memory(task: str, n: int = 3) -> str:
    ...
    Merges ChromaDB operational memory with Mem0 persistent facts.
    """
    blocks = []
    try:
...
class Commander:
    ...
    def _route(self, user_input: str, sender: str,
        ...
        # Retrieve relevant facts from Mem0 persistent memory
        mem0_context = ""
        try:
            from app.memory.mem0_manager import search_shared
            mem0_results = search_shared(user_input, n=3)
            if mem0_results:
                facts = [r.get("memory", "")[:200] for r in mem0_results
