...
| **Headline scorecard (Phase 9, PASSED ✅)** | **6 STRONG** Butlin indicators: **GWT-2, GWT-3, GWT-4, HOT-3, AST-1, PP-1** · **4 PARTIAL**: RPT-2, GWT-1, HOT-2, **AE-1** ("Goals are still user-dispatched, not autonomously generated — hence PARTIAL" — see §3.G1) · **4 ABSENT-by-declaration**: RPT-1, HOT-1, HOT-4, AE-2 · **5 RSM** signatures (4 STRONG + 1 PARTIAL) · **6/6 SK** eval tests passing | `app/subia/README.md`, `app/subia/probes/SCORECARD.md` |
...
This is the **exact gap the SCORECARD already flags**. AE-1 (Agency / Embodiment-1) is rated PARTIAL with the following SCORECARD justification: *"Feedback-driven learning exists via belief asymmetric updates + accuracy-driven cache eviction + retrospective memory promotion. **Goals are still user-dispatched, not autonomously generated — hence PARTIAL**."* Closing G1 is the path from AE-1 PARTIAL → STRONG.
